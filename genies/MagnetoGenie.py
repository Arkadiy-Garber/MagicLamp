...
# ****************************** FILTERING OUT LIKELY FALSE POSITIVES *************************************
    ...
    os.system("mv %s/summary-3.csv %s/magnetogenie-summary.csv" % (args.out, args.out))
    ...
    # GENE COUNTS-BASED ABUNDANCE
        ...
        outHeat = open("%s/magnetogenie.heatmap.csv" % (outDirectory), "w")
        outHeat.write("X" + ',')
        for i in sorted(Dict.keys()):
            outHeat.write(i + ",")
        outHeat.write("\n")
    ...
    if args.makeplots:
        print("Running Rscript to generate plots. Do not be alarmed if you see Warning or Error messages from Rscript. "
              ...
              "If not, you can plot the data as you wish on your own, or start an issue on MagnetoGenie's GitHub repository\n")
        # if conda == 0:
        #     Rdir = args.R
        # else:
        ...
        if args.norm:
            os.system("Rscript --vanilla %s/DotPlot.R %s/magnetogenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/magnetogenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
        else:
            os.system("Rscript --vanilla %s/DotPlot-nonorm.R %s/magnetogenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/magnetogenie.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
    ...
    if args.makeplots:
        ...
        if args.bam == "NA" and args.bams == "NA":
            ...
            os.system("Rscript --vanilla %s/DotPlot.R %s/magnetogenie.readDepth.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))
            os.system("Rscript --vanilla %s/dendro-heatmap.R %s/magnetogenie.readDepth.heatmap.csv %s/" % (rscriptDir, outDirectory, outDirectory))

    print("")
    print("Results are written to %s/magnetogenie-summary.csv and %s/magnetogenie.heatmap.csv" % (args.out, args.out))
