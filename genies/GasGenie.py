...
def main():
    ...
    os.system("echo ${gas_hmms}/hmm-meta.txt > HMMlib.txt")
    ...
    if conda == 0:
        if args.hmm_dir != "NA":
            ...
            print(
                "You have not provided the location of the HMM library via the -hmm_dir argument. Please do so, and try "
    ...
    for i in bits:
        ...
        bitDict[ls[0]]["gene"] = ls[2]
    print("...")

    os.system("mkdir -p %s" % args.out)
    count = 0
    BinDict = defaultdict(lambda: defaultdict(lambda: 'EMPTY'))
    out = open("%s/gasgenie.csv" % (args.out), "w")
    out.write(
        "bin" + "," + "hmm" + "," + "gene" + "," + "ORF" + "," + "evalue" + "," + "bitscore" + "," + "sequence" + "\n")
    for i in binDirLS:
        ...
        if not re.match(r'^\.', i) and lastItem(i.split(".")) == args.bin_ext:
            ...
            for key in HMMdict.keys():
                out.write(
                    i + "," + HMMdict[key]["hmm"] + "," + HMMdict[key]["gene"] +
    ...
    summary = open("%s/gasgenie.csv" % (args.out), "r")
    for i in summary:
        ls = i.rstrip().split(",")
        if ls != "bin":
            genome = ls[0]
            ...
            summaryDict[genome][orf]["hmm"] = hmm
    ...
    print(".")
    out = open("%s/gasgenie-2.csv" % (args.out), "w")
        ...
        for j in CoordDict[i]:
            ...
            for k in clusters:
                if len(RemoveDuplicates(k)) == 1:
                    ...
                    out.write(i + "," + orf + "," + summaryDict[i][orf]["gene"] + "," + summaryDict[i][orf]["hmm"] + "," +
                              summaryDict[i][orf]["bitscore"] + "," + summaryDict[i][orf]["bitcut"] + "," +
                    ...
                    for l in RemoveDuplicates(k):
                        ...
                        out.write(
                            i + "," + orf + "," + summaryDict[i][orf]["gene"] + "," + summaryDict[i][orf]["hmm"] + "," +
                            summaryDict[i][orf]["bitscore"] + "," + summaryDict[i][orf]["bitcut"] + "," +
                            str(summaryDict[i][orf]["sequence"]) + "," + str(counter) + "\n")
    ...
    summary = open("%s/gasgenie-2.csv" % (args.out), "r")
    for i in summary:
        print(i.rstrip())
        if not re.match(r'#', i):
            ls = i.rstrip().split(",")
    ...
    print("..")
    print("...")
    out = open("%s/gasgenie-3.csv" % (args.out), "w")
    out.write(
        "genome" + "," + "orf" + "," "gene" + "," "hmm" + "," "bit_score" + "," "bit_score_cutoff" + "," "AA_seq" + "," + "cluster_id" + "\n")
    ...
    os.system("rm %s/gasgenie.csv" % (args.out))
    os.system("rm %s/gasgenie-2.csv" % (args.out))
    os.system("mv %s/gasgenie-3.csv %s/gasgenie-summary.csv" % (args.out, args.out))
    ...
    # print(".......")
    # print("Finished!")
    # print("results are in %s/gasgenie-summary.csv" % (args.out))
    ...
    if args.bams != "NA":
        ...
        for i in final:
            ls = (i.rstrip().split(","))
            if ls[0] != "bin" and ls[1] != "assembly" and ls[0] != "genome" and ls[0] != "file":
                if not re.match(r'#', i):
                    cell = ls[0]
                    orf = ls[1]
                    gene = ls[2]
